...
def build_graph(reader,
  ...
  tower_distill_labels = tf.split(distill_labels_batch, num_towers)
      ...
      with (tf.variable_scope(("tower"), reuse=True if i > 0 else None)):
        with (slim.arg_scope([slim.model_variable, slim.variable], device="/cpu:0" if num_gpus!=1 else "/gpu:0")):
          ...
          if "loss" in result.keys():
            ...
            label_loss_fn = losses.CrossEntropyLossDistill()
            label_loss = label_loss_fn.calculate_loss(predictions, tower_labels[i], tower_distill_labels[i], p = FLAGS.distill_percent)

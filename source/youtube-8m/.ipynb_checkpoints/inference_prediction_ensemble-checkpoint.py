...
if __name__ == '__main__':
  ...
  flags.DEFINE_integer("file_size", 10000,
...
def inference(all_readers, train_dir, all_data_patterns, out_file_location, batch_size, top_k):
...
def write_to_record(video_ids, video_labels, video_features, filenum, num_examples_processed):
    ...
    input_file_name = os.path.basename(all_data_patterns[0])[:-9]
    writer = tf.python_io.TFRecordWriter(FLAGS.output_dir + '/' + input_file_name + '_ensemble.tfrecord')
...
def main(unused_argv):
  ...
  for i in xrange(len(all_data_patterns)):
    reader = readers.EnsembleReader(
        feature_names=["predictions"], feature_sizes=["3862"])

...
def write_to_record(video_ids, video_labels, video_rgbs, video_audios, video_predictions, video_num_frames, filenum, num_examples_processed):
    input_file_name = os.path.basename(FLAGS.prediction_data_pattern)[:-21]
    writer = tf.python_io.TFRecordWriter(FLAGS.output_dir + '/' + input_file_name + '_distill.tfrecord')

...
class Trainer(object):
  ...
  def __init__(self, cluster, task, train_dir, model, all_readers, model_exporter,
...
#                           task_as_string(self.task))
    ...
    with tf.Graph().as_default() as graph:
      ...
      with tf.device(device_fn):
        if not meta_filename:
          saver = self.build_model(self.model, self.all_readers)
  ...
  def build_model(self, model, all_readers):
    ...
    build_graph(all_readers=all_readers,
                 ...
                 all_data_patterns=all_data_patterns,
